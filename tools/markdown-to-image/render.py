#!/usr/bin/env python3
"""
markdown-to-image renderer — Aria Study integration
Renders Markdown notes as card-style PNG images.
Usage: .venv/bin/python render.py <file.md> [--theme light|dark|warm|forest] [--font sans|serif]
"""
...
THEMES = {
    "light":  {"bg":"#ffffff","text":"#1a1a1a","hl":"#7c5cbf","codeBg":"#f5f5f5","codeText":"#333333","icBg":"#f0f0f0","icText":"#c44444"},
    "dark":   {"bg":"#1a1a2e","text":"#e8e8f0","hl":"#a78bfa","codeBg":"#0d0d1e","codeText":"#a8d8a8","icBg":"#0d0d1e","icText":"#ff9e9e"},
...
FONTS = {
    ...
    "serif": {
        "stack": '"Source Han Serif SC","Noto Serif CJK SC","STSong","SimSun",serif',
...
# ── Shared JS ────────────────────────────────────────────────────────────────
NODE_HTML_JS = """<!-- injected at render time -->"""
NODE_HTML_JS = r"""
...
def paginate(nodes, th, page):
    ...
    if title_node:
        fixed_h += page.evaluate("(text) => measureText(text)", f'<div style="font-size:{th["szTitle"]};font-weight:{th["wTitle"]};line-height:1.25;margin-bottom:10px">{title_node["text"]}</div>')
    if img_node:
        img_h = page.evaluate("(html) => measureTextAsync(html)", f'<img src="{img_node["src"]}" style="max-width:100%;max-height:{COVER_IMG_MAX_H}px;width:auto;height:auto;border-radius:6px;margin:0 auto 9px;display:block">')
        fixed_h += img_h

    heights = page.evaluate("([nodes,th]) => measureNodes(nodes,th)", [remaining, th])
    ...
    while idx < len(overflow):
        ...
        while idx < len(overflow):
            ...
            if n["type"] == "img":
                ...
                if h + img_max + 10 <= MAX_BODY_H:
                    cur.append(capped); h += img_max + 10; idx += 1
                elif not cur:
                    cur.append(capped); idx += 1
                break
            nh = page.evaluate("([n,th]) => measureNodes(n,th)", [[n], th])[0]
            if h + nh <= MAX_BODY_H:
...
def build_card_html(pg, idx, total, th, base_style, img_dir=None):
    ...
    if pg["type"] == "cover":
        ...
        if img:
            ...
            static+=f'<img src="{cover_src}" style="max-width:100%;max-height:{COVER_IMG_MAX_H}px;width:auto;height:auto;border-radius:6px;margin:0 auto 9px;display:block">'
...
def main():
    parser=argparse.ArgumentParser(description="Render Markdown as card-style PNG images")
    parser.add_argument("file", help="Path to the markdown file")
    parser.add_argument("--theme",default="light",choices=list(THEMES.keys()), help="Card theme")
    parser.add_argument("--font",default="sans",choices=list(FONTS.keys()), help="Font family")
    parser.add_argument("--scale",default=8,type=int, help="Device scale factor (default 8 = 2160x2880)")
    ...
    th={**THEMES[args.theme], **{k:v for k,v in font_cfg.items() if k!="stack"}, "fontStack":font_cfg["stack"], "bodyImgMaxH":BODY_IMG_MAX_H}
    base_style=f'*{{box-sizing:border-box;margin:0;padding:0;}}body{{font-family:{font_cfg["stack"]};}}'
    nodes=parse_md(md_path.read_text(encoding="utf-8"))
    print(f"📄 {md_path.name} ({len(nodes)} nodes)  theme: {args.theme}  font: {args.font}  scale: {args.scale}x")

    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        print("❌ Please run setup first"); sys.exit(1)
    ...
    with sync_playwright() as pw:
        ...
        mp.add_script_tag(content=NODE_HTML_JS)
        tmp_measure.unlink()
        print("📐 Measuring node heights...")
        pages=paginate(nodes,th,mp)
        mp.close()
        print(f"📦 {len(pages)} card(s), rendering...")
